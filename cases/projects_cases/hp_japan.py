...
class Project_HPJapan(HPTestCase, Attach):
	...
	FUJINOMIYA_LINK			= '%s/project/47-fujinomiya-project' % URL_BASE_JP
	...
	def __test_main_touts(self):
		...
		for n in range(len(touts_items)):
			...
			self.assertEqual(URL_BASE_JP + '/projects/img/pid/39/dim/285x290/type/' + i[3] + '/crop/1/', imgs[n].get_attribute('src'))
	...
	def __test_footer(self):
		...
		self.assertEqual('%s/resources/images/project-japan/british-council-logo.png' % URL_BASE_JP, logo_img[0].get_attribute('src'))
	...
	def test_index(self):
		...
		self.assertEqual('%s/resources/images/project-japan/historypin-logo.png' % URL_BASE_JP, nav_links[0].e('img').get_attribute('src'))
		self.assertEqual('http://www.historypin.jp/', nav_links[0].get_attribute('href'))
		
		self.assertEqual('%s/jp%s/' % (URL_BASE_JP, self.PROJECT_URL), nav_links[1].get_attribute('href'))
		self.assertEqual(u'ホーム', nav_links[1].text)
		
		self.assertEqual('%s/jp%s/explore/#|map/' % (URL_BASE_JP, self.PROJECT_URL), nav_links[2].get_attribute('href'))
		self.assertEqual(u'探索', nav_links[2].text)
		
		self.assertEqual('%s/jp%s/upload/' % (URL_BASE_JP, self.PROJECT_URL), nav_links[3].get_attribute('href'))
		...
		self.assertEqual('%s/jp%s/user/' % (URL_BASE_JP, self.PROJECT_URL), user_links[0].get_attribute('href'))
		self.assertEqual(u'アカウント作成', user_links[0].text)
		
		self.assertEqual('%s/jp%s/user/' % (URL_BASE_JP, self.PROJECT_URL), user_links[1].get_attribute('href'))
		...
		for n in range(len(button_items)):
			i = button_items[n]
			self.assertEqual(URL_BASE_JP + '/jp' + self.PROJECT_URL + i[0], buttons_links[n].get_attribute('href'))
			self.assertEqual(i[1], buttons_links[n].e('span').text)
		
		self.assertEqual('%s/resources/images/project-japan/home-bg.jpg' % URL_BASE_JP, self.e('.intro img').get_attribute('src'))
	...
	def test_explore(self):
		...
		self.assertEqual('%s/jp/attach%s/map/index/' % (URL_BASE_JP, self.PROJECT_URL), self.e('#embed-frame').get_attribute('src'))
